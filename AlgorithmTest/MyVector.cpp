...
int MyVectorTest(void)
{
    ...
    for (vector<int>::iterator it = vecInt.begin(); it != vecInt.end(); ++it) //迭代器iterator可以指向容器里面的内容
    {
        int iItem = *it;
...
void print_vec1(const vInt & v)//方法一，采用下标访问  
{
    int i;
    for (i = 0; i<v.size(); i++)
    {
...
void print_vec2(const vInt &v)//方法二，采用迭代器访问  
{
